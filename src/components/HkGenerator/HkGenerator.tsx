...
export const HkGenerator: React.FC = () => {
  ...
  const fieldIcon: Record<string, string> = { kontaktart: 'contact_phone', sd: 'fact_check', id: 'verified_user' };

  const sectionHeader = (title: string, category: 'kontaktart' | 'sd' | 'id') => (
    <div className="mb-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <i className={`material-icons transition ${fieldEnabled[category] ? 'text-orange-400' : 'text-neutral-600'}`} style={{ fontSize: '18px' }}>{fieldIcon[category]}</i>
