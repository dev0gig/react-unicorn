...
const NavItem = memo(({ item, isActive, onClick, count }: {
    ...
    <li
        onClick={onClick}
        className={`relative flex items-center rounded-lg cursor-pointer transition-colors duration-200 my-1 p-3
                ${isActive
                ? 'text-white'
    ...
    >
        ...
        <i className="material-icons z-10 mr-4">{item.icon}</i>
...
export const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, onFavoritesClick, onExportClick, onImportClick, onDeleteClick }) => {
    ...
    return (
        <aside className="bg-neutral-800 flex flex-col h-screen flex-shrink-0 border-r border-neutral-700 w-80">
