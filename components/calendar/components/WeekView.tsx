...
const START_HOUR = 7; // Hour to scroll to by default
...
export const WeekView: React.FC<WeekViewProps> = ({ startDate, events }) => {
  ...
  useEffect(() => {
    if (scrollContainerRef.current) {
      // Scroll to 7 AM on initial load of the view
      scrollContainerRef.current.scrollTop = START_HOUR * HOUR_HEIGHT_PX;
    }
  }, [startDate]); // Rerun when the week changes
    ...
    function packColumns(columns: PositionedEvent[][]) {
        ...
        for (let i = 0; i < numCols; i++) {
            for (const event of columns[i]) {
                const top = (event.dtstart.getHours() * 60 + event.dtstart.getMinutes()) / 60 * HOUR_HEIGHT_PX;
    ...
    <div ref={scrollContainerRef} className="h-full overflow-y-auto hide-scrollbar bg-neutral-800 rounded-lg relative">
        <div className="relative grid grid-cols-[4rem_1fr]">
            ...
            <div className="sticky top-0 z-20 bg-neutral-800">
                {hours.map(hour => (
                    <div key={hour} className="relative pr-2 text-right border-r border-neutral-700" style={{ height: `${HOUR_HEIGHT_PX}px` }}>
                        {hour > 0 && <span className="text-xs text-neutral-500 relative -top-2">{`${String(hour).padStart(2, '0')}:00`}</span>}
                    </div>
                ))}
            </div>
            ...
            <div className="relative">
                ...
                {/* Events Layer */}
                <div className="relative grid grid-cols-7" style={{ height: `${24 * HOUR_HEIGHT_PX}px` }}>
                    {weekDays.map(day => (
